...
def Get_All_Transactions_View():
    result = db.session.execute(select(Transaction.date, Transaction.amount, Transaction.description, Merchant.simplename, Category.name).select_from(Transaction).join(Category).join(Merchant)).all()
    return result

def Add_Transaction(data):
    new_transaction = Transaction()
    new_transaction.date = data["date"]
