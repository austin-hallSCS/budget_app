...
def Get_All_Transactions_View():
    result = db.session.execute(select(Transaction.date, Transaction.amount, Transaction.description, Merchant.name, Category.name).select_from(Transaction).join(Category).join(Merchant)).all()
    return result
