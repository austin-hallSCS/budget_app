...
def Get_All_Merchants():
    result = db.session.query(Merchant.merchant_name, Category.category_name).join(Category, Category.category_id == Merchant.category_id, isouter=True).all()
    return result

def Get_All_Merchants_Totals():
    result = db.session.query(
        Merchant.merchant_name, func.round(func.sum(
        Transaction.transaction_amount), 2).label("total"), Category.category_name).group_by(
        Merchant.merchant_id).join(
        Transaction, Transaction.merchant_id == Merchant.merchant_id, isouter=True).join(
        Category, Category.category_id == Merchant.category_id, isouter=True).order_by(
